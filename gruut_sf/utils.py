...
from gruut_sf.resources import _DIR

_LOGGER = logging.getLogger("gruut_sf.utils")
...
def find_lang_dir(
    ...
    #. Looking for ``<lang>/lexicon.db`` in each directory in order:
       ...
       * ``search_dirs``
       * ``$XDG_CONFIG_HOME/gruut_sf``
       * A "data" directory next to the gruut_sf module
    ...
    # ${XDG_CONFIG_HOME}/gruut_sf or ${HOME}/gruut_sf
    maybe_config_home = os.environ.get("XDG_CONFIG_HOME")
    if maybe_config_home:
        search_dirs.append(Path(maybe_config_home) / "gruut_sf")
    else:
        search_dirs.append(Path.home() / ".config" / "gruut_sf")

    # Data directory *next to* gruut_sf
